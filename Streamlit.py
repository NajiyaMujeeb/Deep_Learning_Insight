...
if task == "Sentiment Classification":
    ...
    if new_review_text.strip():
        ...
        try:
            if model_option == "Perceptron":
                ...
                with open('backpropogation.pkl', 'rb') as file:
                    model = pickle.load(file)
            ...
            st.error(f"Error loading the selected model: {e}")
            st.stop()
    ...
    st.sidebar.subheader("Tumor Detection")
